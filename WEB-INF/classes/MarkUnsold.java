...
@WebServlet("/MarkUnsold")

public class MarkUnsold extends HttpServlet
{
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException,IOException
	{
		...
		if(session.getAttribute("username") == null || !session.getAttribute("usertype").toString().equals("seller")){
			request.setAttribute("msg", "Please Login as a Seller to Mark the products unsold");
		...
		{
			if(MySqlDataStoreUtilities.deleteFromTradedGoods(title)) {
				sb.append("<p style='color:green; font-size:20px; margin-bottom:10px;'>Product '"+title+"' marked as unsold.</p>");
				//update the hashmap
