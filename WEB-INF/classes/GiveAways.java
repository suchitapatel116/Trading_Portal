...
@WebServlet("/GiveAways")

public class GiveAways extends HttpServlet
{
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	{
		...
		sb.append("<div class='section-header text-center'><h2 class='title'>Current Give Aways</h2></div>");

		List<Product> giveAways_list = new ArrayList<>();
		giveAways_list = ProductsHashMap.getGiveAwaysList();

		Product product;
		if(giveAways_list == null || giveAways_list.size() == 0)
		{
			sb.append("<p style='font-size:20px; margin-bottom:10px;'>Currently there are no GiveAways. Please come back later.</p>");
		}
		else
		{
			for(int i=0; i<giveAways_list.size(); i++)
			{
				product = (Product)giveAways_list.get(i);
				...
				sb.append("<div class='col-md-4'>");
					sb.append("<div class='blog'>");
						...
						sb.append("</ul><h3>"+product.getTitle()+"</h3>");
							...
							sb.append("<p>Subscribe to qualify for giveaway.</p>");
						...
						sb.append("<input type='hidden' name='phighestbidby' value='"+product.getHighestBidBy()+"'>");
						sb.append("<input type='hidden' name='markSold' value='"+product.getMarkSold()+"'>");
						sb.append("<input type='submit' name='wishListBtn' class='main-btn' value='Subscribe'>");
						sb.append("</form>");
